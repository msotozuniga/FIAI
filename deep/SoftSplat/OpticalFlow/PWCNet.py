...
class PWCNet(torch.nn.Module):
        ...
        class Decoder(torch.nn.Module):
            ...
            def Backward(self, tensorInput, tensorFlow, Backward_tensorGrid, Backward_tensorPartial):
                ...
                tensorOutput = torch.nn.functional.grid_sample(input=tensorInput, grid=(
                            Backward_tensorGrid[str(tensorFlow.size())] + tensorFlow).permute(0, 2, 3, 1),
                ...
                if objectPrevious is None:
                    ...
                    tensorVolume = torch.nn.functional.leaky_relu(
                        input=correlation.FunctionCorrelation(tensorFirst=tensorFirst,
                                                              tensorSecond=self.Backward(tensorSecond,
                                                                                         tensorFlow * self.dblBackward,
                                                                                         Backward_tensorGrid,
    ...
    def forward(self, tensorFirst, tensorSecond):
        tensorFirst = self.in_normalize(tensorFirst)
        tensorSecond = self.in_normalize(tensorSecond)
    ...
    def forward_pre(self, tensorFirst, tensorSecond):
        ...
        objectEstimate = self.moduleSix(tensorFirst[-1], tensorSecond[-1], None, Backward_tensorGrid,
                                        Backward_tensorPartial)
        objectEstimate = self.moduleFiv(tensorFirst[-2], tensorSecond[-2], objectEstimate, Backward_tensorGrid,
                                        Backward_tensorPartial)
        objectEstimate = self.moduleFou(tensorFirst[-3], tensorSecond[-3], objectEstimate, Backward_tensorGrid,
                                        Backward_tensorPartial)
        objectEstimate = self.moduleThr(tensorFirst[-4], tensorSecond[-4], objectEstimate, Backward_tensorGrid,
                                        Backward_tensorPartial)
        objectEstimate = self.moduleTwo(tensorFirst[-5], tensorSecond[-5], objectEstimate, Backward_tensorGrid,
                                        Backward_tensorPartial)
...
if __name__ == '__main__':
    flownet = PWCNet()
    flownet.load_state_dict(torch.load('./pwc-checkpoint.pth'))
